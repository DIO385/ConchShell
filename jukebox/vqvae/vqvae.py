...
class VQVAE(nn.Module):
    def __init__(self, input_shape, levels, downs_t, strides_t,
        ...
        """
        :param input_shape: [样本长度, 1] eg:[16*48k, 1]
        :param levels:
        :param downs_t:
        :param strides_t:
        :param emb_width:
        ...
        self.x_shape = x_shape  # eg: [16k*16]
        ...
        for level in range(levels):
            ...
            # print("check decoder",level, decoder(level))
    ...
    def preprocess(self, x):  # [16, 44100, 1]
        # x: NTC [-1,1] -> NCT [-1,1]
        assert len(x.shape) == 3
        x = x.permute(0, 2, 1).float()  # [16, 1, 44100]
        return x
    ...
    def _decode(self, zs, start_level=0, end_level=None):
        ...
        # print("check xs_quantised", len(xs_quantised), xs_quantised[0], t.min(xs_quantised[0]), t.max(xs_quantised[0]))
        assert len(xs_quantised) == end_level - start_level

        # Use only lowest level
        decoder, x_quantised = self.decoders[start_level], xs_quantised[0:1]
        # print("check another quantizsed", len(xs_quantised), xs_quantised[0], t.min(xs_quantised[0]), t.max(xs_quantised[0]))
        # exit()
        x_out = decoder(x_quantised, all_levels=False)
        x_out = self.postprocess(x_out)
        return xs_quantised, x_out
    ...
    def decode(self, zs, start_level=0, end_level=None, bs_chunks=1):
        ...
        for i in range(bs_chunks):
            ...
            _, x_out = self._decode(zs_i, start_level=start_level, end_level=end_level)
            x_outs.append(x_out)
    ...
    def _encode(self, x, start_level=0, end_level=None):
        ...
        if end_level is None:
            end_level = self.levels
        x_in = self.preprocess(x)  # [16, 1, 44100]
        xs = []
        for level in range(self.levels):
            encoder = self.encoders[level]
            x_out = encoder(x_in)
            xs.append(x_out[-1])
        # print("encoding before bottleneck",xs.size())
        zs = self.bottleneck.encode(xs)
        # print("encoding after bottleneck",zs.size())
        return xs, zs[start_level:end_level]

    def encode(self, x, start_level=0, end_level=None, bs_chunks=1):
        x_chunks = t.chunk(x, bs_chunks, dim=0)
        zs_list = []
        for x_i in x_chunks:
            _, zs_i = self._encode(x_i, start_level=start_level, end_level=end_level)
            zs_list.append(zs_i)
