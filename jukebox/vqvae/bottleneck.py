...
class BottleneckBlock(nn.Module):
    ...
    def quantise(self, x):
        ...
        k_w = self.k.t()  # this is the learned codebook in size of (64, 2048)
        # print("check x and quantise layer", x.size(), k_w.size())
        # print("check codebook", k_w[:,0], k_w[:,66])

        distance = t.sum(x ** 2, dim=-1, keepdim=True) - 2 * t.matmul(x, k_w) + t.sum(k_w ** 2, dim=0,
                                                                                      keepdim=True)  # (N * L, b)
        min_distance, x_l = t.min(distance, dim=-1)  # x_l is the index of codebook entries, 5512
        # print("check min_distance and x_l", min_distance, x_l.size(), x_l)
...
class Bottleneck(nn.Module):
    def __init__(self, l_bins, emb_width, mu, levels):
        ...
        # self.level_block_top = nn.ModuleList()
        for level in range(self.levels):
            self.level_blocks.append(level_block(level))
        # self.level_block_top.append(level_block(2))

    def encode(self, xs):
        # print("&&&check level block&&&")
        # print(self.level_blocks)
        zs = [level_block.encode(x) for (level_block, x) in zip(self.level_blocks, xs)]
        return zs

    # def encode_top(self, xs):
    #    zs = [level_block.encode(x) for (level_block, x) in zip(self.level_block_top, xs)]
    #    return zs
